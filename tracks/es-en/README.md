...
* **Inference Strategy:**
  * Decoding configuration: Greedy search (`num_beams=1`), FP16 mixed precision.
  * Task: Automatic code-switching transcription (`task="transcribe"`).
  * Post-Processing: Custom bilingual normalizer combining `BasicTextNormalizer`, conversational contraction joiner (`didn't`, `i'm`), and spontaneous stutter-reduction filters.
* **Weights Storage:** Checkpoint adapter weights (`adapter_model.safetensors`) are hosted privately on Google Drive / Kaggle Datasets in accordance with competition data sharing rules.
...
2. **Code-Switching Boundary Stability:** The LoRA adapter successfully prevents decoder looping on intra-sentential language switches (Spanish $\leftrightarrow$ English), retaining bilingual word choices without hallucinatory script drift.
